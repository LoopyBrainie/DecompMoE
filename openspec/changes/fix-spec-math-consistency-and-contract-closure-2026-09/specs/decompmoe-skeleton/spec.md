...
### Requirement: Centroid Four-Phase Lifecycle Driver  (MODIFIED)
...
### Requirement: Spherical L2 Normalization  (MODIFIED)
...
- **THEN** the result's `pow(2).sum(-1) ∈ [1 − 2ε, 1]` (the upper bound `1.0` is attained when `‖z‖₂ ≥ ε`; the lower bound `1 − 2ε` follows from the Lipschitz continuity of `1/x` near `‖z‖₂ = 1 − ε`)

#### Scenario: Idempotence

- **WHEN** the function is applied twice in succession
- **THEN** the second application leaves the output unchanged (since the output norm is `≤ 1`, the second `max(‖·‖, ε)` denominator is `1.0` exactly, giving the identity map)

---

### Requirement: Beta Parameterization Operational Domain  (MODIFIED)
...
### Requirement: Frozen MVP Hyperparameter Set  (MODIFIED)
...
### Requirement: Eight Metrics And Classification  (MODIFIED)
